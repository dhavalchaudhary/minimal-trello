...
export const App = () => {
  const [categories, setCategories] = usePersistedState<CategoryType[]>(
    Entity.CATEGORY,
    []
  )
  ...
  const addCard: CardContextType['handlers']['addCard'] = (
    title,
    categoryIndex
  ) => {
    const newCard: Card = {
  ...
  const updateCard: CardContextType['handlers']['updateCard'] = (
    cardId,
    data
  ) => {
  ...
  const moveCard = (
    ...
    newCategoriesState = [
      ...newCategoriesState.slice(0, currentCategoryIndex),
      newCurrentCategoryData,
      ...newCategoriesState.slice(currentCategoryIndex + 1)
    ]
    ...
    newCategoriesState = [
      ...newCategoriesState.slice(0, expectedCategoryIndex),
      newExpectedCategoryData,
      ...newCategoriesState.slice(expectedCategoryIndex + 1)
    ]
