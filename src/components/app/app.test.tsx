...
describe('App', () => {
  ...
  it('renders all the categories', () => {
    ...
    expect(screen.getAllByTestId('category-column')).toHaveLength(
      mockCategoriesData.length
    )
  })
  it('should create a new category', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/add category title/i), {
      target: { value: mockCategoryTitle }
    })
  ...
  it('should create a new card within a category', () => {
    loadlocalStorage({
      categories: mockCategoriesData.slice(0, 1),
      cards: mockCardsData
    })
    ...
    const existingCardsLength =
      screen.getByTestId('card-list-wrapper').children.length

    fireEvent.click(screen.getByText(/add new card/i))
    fireEvent.change(screen.getByPlaceholderText(/add card title/i), {
      target: { value: mockCardTitle }
    })
    fireEvent.click(screen.getByTestId('entity-add-save'))

    expect(screen.getByText(mockCardTitle)).toBeVisible()
    expect(screen.getByTestId('card-list-wrapper').children).toHaveLength(
      existingCardsLength + 1
    )
  })
  it('should persist the newly created data', () => {
    ...
    const persistedCardsData = JSON.parse(
      window.localStorage.getItem(Entity.CARD) || ''
    )
    const persistedCategoriesData = JSON.parse(
      window.localStorage.getItem(Entity.CATEGORY) || ''
    )
  ...
  it('should update a card within a category', () => {
    ...
    fireEvent.change(screen.getByTestId('card-edit-input'), {
      target: { value: mockNewCardTitle }
    })
  ...
  it('should move a card to different columns', () => {
    loadlocalStorage({
      categories: mockCategoriesData.slice(0, 2),
      cards: mockCardsData
    })

    const getCategoryCardsLength = (index: number) =>
      screen
        .getAllByTestId('category-column')
        [index].querySelector('[data-testid="card-list-wrapper"]')?.children
        .length as number
